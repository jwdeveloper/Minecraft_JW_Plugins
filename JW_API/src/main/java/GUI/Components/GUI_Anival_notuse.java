...
import Packets.Wrapper.VersionMatcher;
import Packets.Wrapper.VersionWrapper;
 ...
 /*   new GUI_Anival.Builder()
                                                ...
                                                .onComplete((player, text) -> {           //called when the inventory output slot is clicked
                                                    if(text.equalsIgnoreCase("you")) {
                                                        ...
                                                        return GUI_Anival.Response.close();
                                                    } else {
                                                        return GUI_Anival.Response.text("Incorrect.");
                                                    }
                                                })
                                                ...
                                                .plugin(Bukkit.getPluginManager().getPlugins()[0])                 //set the plugin instance
                                                .open(clicker);*/
public class GUI_Anival_notuse {
     ...
     * @deprecated As of version 1.2.3, use {@link GUI_Anival_notuse.Builder}
     */
    @Deprecated
    public GUI_Anival_notuse(Plugin plugin, Player holder, String insert, BiFunction<Player, String, String> biFunction, BiFunction<Player, String, Response> completeFunction, Plugin plugin1, Consumer<Player> closeListener) {
    ...
    public GUI_Anival_notuse(Plugin plugin, Player player, boolean preventClose, Consumer<Player> closeListener, BiFunction<Player, String, Response> completeFunction) {
        this.plugin = plugin;
        this.player = player;
        this.preventClose = preventClose;
        this.closeListener = closeListener;
    ...
    private class ListenUp implements Listener {
        ...
        public void onInventoryClose(InventoryCloseEvent event) {
            if (open && event.getInventory().equals(inventory)) {
                closeInventory();
                if(preventClose) {
                    Bukkit.getScheduler().runTask(plugin, GUI_Anival_notuse.this::openInventory);
                }
            }
        }
    ...
    /**
     * A builder class for an {@link GUI_Anival_notuse} object
     */
    public static class Builder {
        ...
        /**
         * Sets the plugin for the {@link GUI_Anival_notuse}
         ...
         * @return The {@link GUI_Anival_notuse} instance from this builder
         * @throws IllegalArgumentException when the onComplete function, plugin, or player is null
         */
        public GUI_Anival_notuse open(Player player) {
            Validate.notNull(plugin, "Plugin cannot be null");
            Validate.notNull(completeFunction, "Complete function cannot be null");
            Validate.notNull(player, "Player cannot be null");
            return new GUI_Anival_notuse(plugin, player, title, itemText, item, preventClose, closeListener, completeFunction);
        }
